...
void csm_receiveIdle(void) {
    ...
    csm_state.threshold = csm_state.lastRx.elapsedTime * 4 / 10;

    if (csm_state.curRx.signal == 0 &&
        csm_isWithinThreshold(csm_state.curRx.elapsedTime / 2,
                                csm_state.lastRx.elapsedTime)) {
...
void csm_receiveData(void) {
    ...
    else if (csm_isWithinThreshold(csm_state.curRx.elapsedTime / 2,
                                csm_state.deltaT)) {
...
void csm_finishAdvanceTransmitState(void) {
    txTempBit = csm_state.txDispatch[csm_state.txState]();
